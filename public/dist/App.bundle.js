...
function autocomplete(input, latInput, lngInput) {
	if (!input) return; // skip this function from running if there is no input onthe page
	...
	// If someone hits the enter key on the address field, do not submit the form
...
function makeMapFr(mapDiv) {
	if (!mapDiv) return;
	// Making the map
	var map = new google.maps.Map(mapDiv, mapOptionsFr);
	loadPlacesFr(map);
	var input = (0, _bling.$)('[name="geolocatefr"]');
	var autocomplete = new google.maps.places.Autocomplete(input);
	autocomplete.addListener('place_changed', function () {
		var place = autocomplete.getPlace();
...
function typeAheadFr(search) {
	...
	var searchInputFr = search.querySelector('input[name="search_fre"]');
	var searchResultsFr = search.querySelector('.searchfr__results');
	...
	searchInputFr.on('input', function () {
		...
		_axios2.default.get('/api/search?q=' + this.value).then(function (res) {
			if (res.data.length) {
				searchResultsFr.innerHTML = _dompurify2.default.sanitize(searchResultsHTMLFr(res.data));
				return;
			}
			// Tell the users nothing came back
			searchResultsFr.innerHTML = _dompurify2.default.sanitize('<div class="searchfr__result"> Aucun r\xE9sultat pour ' + _this.value + ' trouv\xE9</div>');
		}).catch(function (err) {
			console.error(err);
		});
	});
	...
	// handle keyboard inputs
	searchInputFr.on('keyup', function (e) {
		// If they are not pressing up, down, or enter key, who cares!
		if (![38, 40, 13].includes(e.keyCode)) {
			return; // Skip it!
		}
		var activeClass = 'searchfr__result--active';
		var current = search.querySelector('.' + activeClass);
		var items = search.querySelectorAll('.searchfr__result');
		var next = void 0;
		if (e.keyCode === 40 && current) {
			next = current.nextElementSibling || items[0];
		} else if (e.keyCode === 40) {
...
var _typeAhead_fr = __webpack_require__(16);

var _typeAhead_fr2 = _interopRequireDefault(_typeAhead_fr);

var _map = __webpack_require__(13);

var _map2 = _interopRequireDefault(_map);

var _map_fre = __webpack_require__(14);

var _map_fre2 = _interopRequireDefault(_map_fre);
...
(0, _typeAhead_fr2.default)((0, _bling.$)('.searchfr'));

(0, _map2.default)((0, _bling.$)('#map'));

(0, _map_fre2.default)((0, _bling.$)('#mapfr'));
